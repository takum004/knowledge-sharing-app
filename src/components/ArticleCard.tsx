...
const ArticleCard: React.FC<ArticleCardProps> = ({
  ...
  return (
    <div className="article-card relative flex gap-4 rounded-lg p-4 hover:shadow-lg hover:-translate-y-1 transition-all duration-200 cursor-pointer bg-white border border-[#e7edf4]">
      {/* 記事情報 */}
      <div className="flex-[2_2_0px] flex flex-col justify-between">
        <div>
          ...
          {/* 記事タイトル */}
          <h3 className="text-base font-bold text-[#0d141c] mb-2 line-clamp-2">
            {title}
          </h3>
          
          {/* 記事説明 */}
          <p className="text-[#49739c] text-sm mb-3 line-clamp-3">
            {description}
          </p>
        ...
        <div className="flex items-center gap-2">
          {/* 読むボタン */}
          <button 
            className="bg-[#e7edf4] text-[#0d141c] rounded-lg h-8 px-4 text-sm font-medium hover:bg-primary hover:text-white transition-colors"
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors ${
              isReadLater 
                ...
                : 'bg-[#e7edf4] text-[#49739c] hover:bg-orange-500 hover:text-white'
            }`}
      ...
      {/* 記事画像 */}
      <div className="flex-1 min-w-[120px] max-w-[200px] aspect-video rounded-lg overflow-hidden bg-gray-100">
        <img 
          src={currentImageUrl} 
          alt={title}
